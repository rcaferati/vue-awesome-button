import type { SharePayload, ShareType } from '../types';
...
function toWindowFeatures(
  ...
  return Object.entries(params)
    .filter(([, value]) => value != null && String(value).trim() !== EMPTY_STRING)
...
function normalizePopupDimensions(
  ...
  return {
    width: clampNumber(width, DEFAULT_POPUP_WIDTH, {
      min: MIN_POPUP_WIDTH,
    }),
...
export function isMobile(): boolean {
  ...
  return MOBILE_USER_AGENT_REGEX.test(navigator.userAgent || EMPTY_STRING);
}
...
function buildPopupExtra(width: number, height: number): string {
  const dimensions = normalizePopupDimensions(width, height);
  const { top, left } = centerWindow(dimensions.width, dimensions.height);
...
function withPopup(
  ...
  dimensions: SharePopupDimensions
): SharePayload {
  const normalized = normalizePopupDimensions(dimensions.width, dimensions.height);
